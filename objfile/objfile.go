...
// Disasm is implemented in disasm.go
// RawDisasm returns the raw disassembler for the file
func (e *Entry) RawDisasm() (*Disasm, error) {
	return e.raw.disasm()
}

func (e *Entry) moduledata() (secStart uint64, moduleData *ModuleData, err error) {
	// Get pclntab candidates
	pclntabCandidates, err := e.raw.pcln()
	if err != nil {
		return 0, nil, fmt.Errorf("failed to get pclntab candidates: %v", err)
	}
	
	// Scan for moduledata candidates
	candidates := make(chan *ModuleDataCandidate)
	go func() {
		defer close(candidates)
		for pclntabCandidate := range pclntabCandidates {
			candidate, err := e.raw.moduledata_scan(pclntabCandidate.PclntabVA, e.is64bit, e.littleendian, nil)
			if err != nil {
				continue
			}
	...
	for candidate := range candidates {
		moduleData, err := ParseModuleData(candidate.Moduledata, e.version, e.is64bit, e.littleendian)
		if err != nil {
			continue
		}
		
		// Validate the parsed data
		if err := ValidateModuleData(moduleData, candidate.ModuledataVA); err != nil {
			continue
		}
		
		return candidate.SecStart, moduleData, nil
	}
	
	return 0, nil, fmt.Errorf("failed to find valid moduledata")
}
